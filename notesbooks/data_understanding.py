...
def summarize_csv(
    ...
    *,
    ...
    When ``missing`` is ``\"sample\"`` (default), ``top_missing`` percentages use only
    the first ``sample_rows`` rows, while ``rows`` is always the full-file row count.
    Use ``missing=\"full\"`` for exact missing rates over the entire CSV (slower).
    """
